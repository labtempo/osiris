...
import br.uff.labtempo.osiris.util.connection.Config;
...
public class RemoteListener <T>{
    ...
    public RemoteListener(String host, String resourceName, T instance, Class<?> klass) throws Exception {
        loadProperties();
        String exchange = prop.getProperty("amqp.exchange");
        String exchangeType = prop.getProperty("amqp.exchange.type");
        String queue = prop.getProperty("amqp.routing.key")+resourceName;
