...
public class VirtualSensorLink extends VirtualSensor {
    ...
    public void update(SensorCoWrapper sensor) {
        if (isSource(sensor)) {
            List<Field> sensorCoFields = sensor.getFields();
            for (Field sensorCoField : sensorCoFields) {
                updateFields(sensorCoField.getName(), sensorCoField.getValue(), sensorCoField.getDefaultDataType());
            }
        }
    }
