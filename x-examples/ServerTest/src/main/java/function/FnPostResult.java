...
public class FnPostResult {
    ...
    public static void deleteAll(OmcpClient client) {
        ...
        requestFnTo.setResponseTo("omcp://virtualsensornet/blending/4");

        Response r = client.doPost(uri,requestFnTo);
        if (r.getStatusCode() == StatusCode.CREATED) {
            System.out.println("CREATED: " + r.getLocation());
        } else {
